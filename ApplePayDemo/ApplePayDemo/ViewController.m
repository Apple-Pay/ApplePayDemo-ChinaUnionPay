...
- (IBAction)buyNow:(id)sender {
    ...
    //检查用户是否可进行某种卡的支付，是否支持Amex、MasterCard、Visa、中国银联四种支付卡，根据自己项目的需要进行检测
    ...
    payRequest.merchantIdentifier = @"merchant.ApplePayDemo";
    //申请的merchantID
    ...
    //账单邮寄地址可以事先让用户选择是否需要，否则会增加客户的输入麻烦度，体验不好，
    ...
    NSDecimalNumber *subtotalAmount = [NSDecimalNumber decimalNumberWithMantissa:10000 exponent:-2 isNegative:NO];   //
    PKPaymentSummaryItem *subtotal = [PKPaymentSummaryItem summaryItemWithLabel:@"商品价格" amount:subtotalAmount];
    
    NSDecimalNumber *discountAmount = [NSDecimalNumber decimalNumberWithString:@"-33.34"];      //-33.34
    ...
    PKPaymentSummaryItem *total = [PKPaymentSummaryItem summaryItemWithLabel:@"Samboo" amount:totalAmount];
    //最后这个是支付给谁。
